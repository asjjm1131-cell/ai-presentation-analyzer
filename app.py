...
def analyze_presenter(image_file):
    ...
    if len(faces) == 0:
        ...
        feedback.append(
            "얼굴이 감지되지 않았습니다. 카메라를 정면으로 바라보세요."
        )
    ...
    if centered:
        ...
        feedback.append(
            "발표자가 화면 중앙에 위치해 있습니다."
        )

    else:

        feedback.append(
            "발표자가 화면 중앙에서 벗어나 있습니다."
        )
    ...
    if eye_detected:
        ...
        feedback.append(
            "눈이 감지되어 시선 처리가 양호한 것으로 판단됩니다."
        )

    else:

        feedback.append(
            "눈 감지가 약합니다. 카메라를 정면으로 바라보는 것이 좋습니다."
        )
    ...
    if smile_detected:
        ...
        feedback.append(
            "웃는 표정이 감지되었습니다. 발표 인상이 긍정적입니다."
        )

    else:

        feedback.append(
            "표정 변화가 적습니다. 자연스러운 표정을 유지하면 좋습니다."
        )
...
if uploaded_file is not None:
    ...
    for page_num in range(len(pdf)):
        ...
        score, reasons = calculate_ppt_score(
            text_count,
            image_count,
            title_exists
        )
        ...
        if score >= 75:
            ...
            danger_class = "badge-good"

        elif score >= 50:

            danger = "주의"

            danger_class = "badge-warn"
    ...
    worst_slide = slide_scores.index(
        min(slide_scores)
    ) + 1
    ...
    avg_text_per_slide = (
        total_text_count / len(slide_data)
    )
    ...
    text_only_ratio = (
        text_only_count / len(slide_data)
    )
    ...
    <div class="card">
        ...
        <p>
        총 예상 발표 시간:
        <b>{total_estimated_time:.1f}분</b>
        </p>

        <p>
        슬라이드당 평균 글자 수:
        <b>{avg_text_per_slide:.0f}자</b>
        </p>

        <p>
        텍스트 위주 슬라이드 비율:
        <b>{text_only_ratio * 100:.1f}%</b>
        </p>

        <p>
        최종 감점/판단 이유:
        <b>{", ".join(final_reasons)}</b>
        </p>
...
if camera_image is not None:
    ...
    result, error = analyze_presenter(
        camera_image
    )
...
if (
    ppt_score is not None
    and presenter_score is not None
):
    ...
    if final_score >= 80:
        ...
        st.success(
            "발표자료와 발표자 상태가 모두 우수합니다."
        )

    elif final_score >= 60:

        st.warning(
            "전체적으로 양호하지만 일부 슬라이드 구성 또는 발표 자세 개선이 필요합니다."
        )

    else:

        st.error(
            "발표자료와 발표자 상태 모두 개선이 필요합니다."
        )

else:

    st.info(
        "PDF 발표자료와 발표자 사진을 모두 입력하면 종합 평가가 출력됩니다."
    )
